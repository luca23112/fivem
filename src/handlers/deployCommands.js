...
(async () => {
  ...
  const mode = config.commandDeployMode;
  if (mode === 'guild') {
    if (!config.guildId) throw new Error('COMMAND_DEPLOY_MODE=guild benötigt GUILD_ID in .env');
    await rest.put(Routes.applicationGuildCommands(config.clientId, config.guildId), { body: commands });
    console.log(`Guild-Commands deployed (${commands.length}) für ${config.guildId}`);
    return;
  }
