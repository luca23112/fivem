...
module.exports = {
  ...
  async execute(client) {
    ...
    try {
      if (config.commandDeployMode === 'guild') {
        if (!config.guildId) throw new Error('COMMAND_DEPLOY_MODE=guild benötigt GUILD_ID.');
        await rest.put(Routes.applicationGuildCommands(config.clientId, config.guildId), { body: commands });
        logger.info(`Slash-Commands (guild) synchronisiert: ${commands.length} für ${config.guildId}`);
      } else {
        await rest.put(Routes.applicationCommands(config.clientId), { body: commands });
        logger.info(`Slash-Commands (global) synchronisiert: ${commands.length}`);
      }
    } catch (err) {
