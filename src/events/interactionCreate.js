...
function ticketControlsRow() {
  return new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId('ticket_add_user').setLabel('User hinzufügen').setStyle(ButtonStyle.Success),
    ...
    try {
      ...
      if (interaction.isAutocomplete()) {
        if (interaction.commandName === 'feature-hub') {
          ...
          const choices = features
            .filter((f) => f.id.includes(focused) || f.name.toLowerCase().includes(focused))
            .slice(0, 25)
            .map((f) => ({ name: `${f.id} • ${f.name}`, value: f.id }));
      ...
      if (interaction.isButton()) {
        if (interaction.customId === 'open_ticket') {
          ...
          db.prepare('INSERT OR REPLACE INTO tickets (channel_id, guild_id, creator_id, type, status) VALUES (?, ?, ?, ?, ?)')
            .run(channel.id, interaction.guild.id, interaction.user.id, 'general', 'open');
          await channel.send({ content: 'Made by Luca', embeds: [baseEmbed('🎫 Ticket erstellt', 'Ein Teammitglied wird sich gleich melden.')], components: [ticketControlsRow()] });
          await interaction.reply({ content: `Ticket erstellt: ${channel}`, ephemeral: true });
        }
        ...
        if (interaction.customId === 'open_captcha_modal') {
          ...
          modal.addComponents(
            new ActionRowBuilder().addComponents(
              new TextInputBuilder().setCustomId('captcha_input').setLabel(`Bitte Code eingeben: ${code}`).setStyle(TextInputStyle.Short).setRequired(true)
            )
          );
