...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const result = db.prepare('INSERT INTO reminders (guild_id, user_id, channel_id, text, remind_at) VALUES (?, ?, ?, ?, ?)')
      .run(interaction.guild.id, interaction.user.id, interaction.channel.id, text, remindAt);

    await interaction.reply({ content: `⏰ Erinnerung #${result.lastInsertRowid} gespeichert. Ich erinnere dich in ${min} Minuten.`, ephemeral: true });
  }
};
